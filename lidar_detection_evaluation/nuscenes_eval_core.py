...
class NuScenesEval:
    ...
    def eval_pair(self, pred_label, gt_label):
        ...
        if pred_label.shape[0] > 0:
            pred_label = pred_label[pred_label[:, 8].astype(float) > self.score_threshold, :]

        for single_class in self.classes:
            # get all pred labels, order by score
            class_pred_label = pred_label[np.char.lower(pred_label[:, 0].astype(str)) == single_class, 1:]
            score = class_pred_label[:, 7].astype(float)
            class_pred_label = class_pred_label[(-score).argsort(), :].astype(float) # sort decreasing

            # add gt label length to total_N_pos
            class_gt_label = gt_label[np.char.lower(gt_label[:, 0].astype(str)) == single_class, 1:].astype(float)
    ...
    def filter_by_range(self, pred_label, gt_label, range=0):
        pred_dist = np.linalg.norm(pred_label[:, 1:4].astype(float32), axis=1) < range
        gt_dist = np.linalg.norm(gt_label[:, 1:4].astype(float32), axis=1) < range
